...
function showTreeComplete() {
  ...
  div.innerHTML = `<span class="alert-icon">✅</span>
    <div><strong>Árbol completado.</strong> Hipótesis identificadas: <strong>${hyps.map(h=>h.name).join(', ')}</strong>. Proceda a confirmar con los tests específicos.</div>`;
  container.appendChild(div);
...
function recalcHypScore(hId) {
  ...
  if (evaluatedCount === 0) {
    color = 'var(--text3)'; label = 'Sin evaluar'; colorClass = 'hyp-orange';
  } else if (totalLR >= 5 || (hasHighLR && positiveCount > 0)) {
    color = 'var(--green)'; label = `🟢 Peso alto (LR× ${totalLR.toFixed(1)})`; colorClass = 'hyp-green';
  } else if (totalLR >= 2 || positiveCount > 0) {
    color = 'var(--orange)'; label = `🟠 Peso moderado (LR× ${totalLR.toFixed(1)})`; colorClass = 'hyp-orange';
  } else {
    color = 'var(--red)'; label = `🔴 Peso bajo (LR× ${totalLR.toFixed(1)})`; colorClass = 'hyp-red';
  }
...
function buildResults() {
  ...
  if (sorted.length === 0) {
    hypHtml += `<p style="color:var(--text3); font-size:0.85rem;">No se identificaron hipótesis diagnósticas.</p>`;
  } else {
