...
- [x] `node tests/unit.js` sigue en verde (48/48) tras añadir las nuevas secciones.

**Notas / decisiones:**
- **Alcance deliberadamente acotado, no un refactor de arquitectura.** Se consideró ir más lejos — inyectar `CIF_TREES`/`HYPOTHESES` como parámetros en todas las funciones de `phase4.js`/`phase4b.js` (en vez de `import` estático a nivel de módulo) para poder testear `selectTreeOption`/`initCIFTree`/`showTreeComplete` también con datos ficticios. Se descartó: esas funciones resuelven `CIF_TREES[state.region]`/`HYPOTHESES[hId]` internamente porque las llaman ~35 atributos `onclick` inline (ver notas de Fase A) que no pueden pasar el árbol/hipótesis como argumento adicional sin reescribir esos handlers — cambio de alcance mucho mayor y no descrito en este plan. `rebuildHypotheses` y `pruneTreeFrom` sí eran testeables con datos ficticios sin tocar una sola línea de `phase4.js`, porque ya recibían `tree` como parámetro; ahí se centró el esfuerzo.
- **Por qué no se testean `renderStep`/`selectTreeOption`/`checkTreeComplete`/`showTreeComplete` con el árbol ficticio:** el shim de DOM de `tests/unit.js` (`document.getElementById: () => makeEl()`) devuelve un elemento nuevo en cada llamada en vez de mantener un registro por id — funciona para las funciones que solo *escriben* en el DOM (asignar `.disabled`, `.className`, llamar `.remove()`), pero no permite verificar después *qué* se renderizó, ni que `document.getElementById(step.id)` reconozca un paso ya pintado (`renderStep` cortocircuita con `if (existing) return`, y `existing` sale siempre truthy con este shim). Convertir el shim en un DOM falso real (con registro de elementos, `appendChild` que realmente añade hijos) haría esas funciones testeables, pero es una pieza de trabajo aparte, no ligada a datos clínicos — se deja fuera de esta fase.
- La validación de integridad de `CIF_TREES` es la pieza de mayor valor práctico: no exhaustiva a nivel de "todo el árbol es alcanzable" (el patrón de navegación es más una batería de cribado secuencial con hipótesis acumulativas que un árbol de decisión estricto — casi todas las opciones, tengan `next` explícito o no, acaban llevando al siguiente paso del array; ver el comentario de esquema sobre `next`), pero sí atrapa exactamente los dos errores que un editor de `data.js` puede cometer sin darse cuenta: un `next` mal escrito (paso inexistente) y un id de hipótesis mal escrito o copiado de otra región.
